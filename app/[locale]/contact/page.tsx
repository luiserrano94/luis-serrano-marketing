...
export function generateMetadata({ params }: { params: { locale: string } }): Metadata {
  ...
  return { title: `${c.contact.title} · Luis Serrano`, description: c.contact.sub, alternates: localeAlternates(params.locale, "/contact") };
}
  ...
  return (
    <section className="wrap sec">
      <div className="contact-head reveal">
        <h1 className="display" style={{ fontSize: "clamp(42px,6.4vw,92px)" }}>{c.contact.title}</h1>
        <p>{c.contact.sub}</p>
